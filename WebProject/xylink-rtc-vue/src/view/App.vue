<template>
  <div id="app">
    <!-- <el-button type="primary" style="position: absolute; top: 0;right: 0;pointer-events: auto;user-select: none;z-index:1;" @click.stop="submitForm()">打开</el-button> -->
    <div id="container" class="containerBox">
      ...
      <div class="meeting" v-if="callMeeting && !callLoading">
        ...
        <div class="meeting-content" id="meeting" @click.stop="handleToolVisible">
          ...
          <div class="videoList" v-if="!isFull">
            <VideoItem
          ...
          <div class="meeting-layout" :style="layoutStyle" v-else>
            <Video
              v-for="(item, index) in layout"
              :key="item.id"
              :id="item.id"
        ...
        <div :class="toolVisible ? 'meeting-footer xy__show' : 'meeting-footer xy__hide'" v-if="false">
          <div class="middle">
...
import VideoItem from './components/Video/list.vue';
...
export default {
  ...
  data() {
    return {
      ...
      templateMode: 'CONTENT_ONLY', //SPEAKER GALLERY 桌面布局模式(语音激励模式、画廊模式)
      ...
      meetingName:'',
      startJoin: false, //是否开始加入会议
      isInVideoMeet: false,// 是否进入会议
      isFull: false,// 是否为全屏
      xyVideoList: [], // 视频会议状态
    };
  },
  mounted(){
    ...
    window.addEventListener('message', (e)=>{
      /* if (e.data.type === 'VUE2_EVENT') {
        console.log('收到 Vue2 消息:', e.data.payload)
      } */
      if(e.data.type === 'startMeet'){
        thisObj.stop()
        thisObj.submitForm()
      }
      if(e.data.type === 'stopMeet'){
        thisObj.stop()
      }
      console.log(e)
    })
    window.wOpenXyLinkFunc = ({meeting,extUserId,meetingName,userName,meetingPassword,isLandscape})=>{
      if(!this.isInVideoMeet){
        this.meetingName = meetingName;
        /* var screenLandscape = 0
        if(window.screen.width>window.screen.height){
          screenLandscape = 1
        } */
        var extId = extUserId
        if(isLandscape){
          extId = extUserId+'_'+isLandscape
        }
        let values ={
          extUserId: extId,
          meeting,
        ...
        try {
          ...
          xyLinkVideoLogic.vueMinStatus(false)
        } catch (error) {
    ...
    // 修改本地视频状态
    window.wChangeXyLinkStateFunc = (data)=>{
      if(data.type=="meet"){
        if(data.value){
          // no have
        }else{
      ...
      if(data.type=="camera"){
        this.video = data.value?'muteVideo':'unmuteVideo'
        this.videoOperate()
      }
      if(data.type=="mic"){
        this.audio = data.value?'muteAudio':'unmuteAudio'
        this.onAudioOperate()
      }
    }
  },
  ...
  watch:{
    xyVideoList: {
      handler(newValue,oldValue) {
        ...
        this.debounceCalcWatchVideData(newValue,oldValue)
      },
      deep: true,
    },
  },
  methods: {
      ...
      for (let i = 0; i < arr1.length; i++) {
        ...
        for (const key in obj1) {
          if (obj1[key] !== obj2[key]) {
            differences.push(i); // 如果发现差异，记录下标
            hasDifference = true;
            break; // 一旦找到差异，就不需要检查其他属性了
          }
        }

        if (hasDifference) {
          // 如果obj2有额外的属性，也需要记录下标
    ...
    debounceCalcWatchVideData: _.debounce(function(n,o){
      this.calcWatchVideData(n,o)
    },500),
    calcWatchVideData(n,o){
      let userIds = ['24104']
      // eslint-disable-next-line no-undef
      if(typeof(dmData) != "undefined" && _.has(dmData,'meetingUser')){
        // eslint-disable-next-line no-undef
        userIds = dmData.meetingUser.map(item=>item.topicUserId)
      }
      // 判断数据length改变
      if(n.length == o.length) {
        // 对比
        let indexs = this.findDifferences(n,o);
        indexs.map(item=>{
          var tId = ""
          try {
            tId = this.xyVideoList[item].extUserId.split("_")[0]
          } catch (error) {
            tId = this.xyVideoList[item].extUserId
          }
          if(!_.includes(userIds,tId)){
            // 通知更改
            try {
              // eslint-disable-next-line no-undef
              xyLinkVideoLogic.uploadLocalVideoData()
            } catch (error) {
              console.log('调用 xyLinkVideoLogic.uploadLocalVideoData() 失败：');
            }
          }
        })
      } else {
    ...
    getRandomIntInclusive(min, max) {
      min = Math.ceil(min);
      max = Math.floor(max);
      return Math.floor(Math.random() * (max - min + 1)) + min; //含最大值，不含最小值
    },
    // 登录
    async submitForm(values) {
      console.log('submitForm',values)
      if(!values) {
        let randNum = this.getRandomIntInclusive(0,100)
        var extId = "24104"+randNum
        if(this.getUrlParam("isLand")!=null){
          extId = extId+'_true'
        }
        // var screenLandscape = window.screen.width>window.screen.height
        values ={
          extUserId: extId,//extId+'_'+screenLandscape
          meeting: "9083679033",//"9083158122"
          meetingName: "指挥"+'_'+randNum,
          meetingPassword: "",
          muteAudio: true,
          muteVideo: false,
          password: "",
    ...
    async join() {
      ...
      try {
         ...
         * 重要提示
         */
        console.log("this.setting.isThird: ",this.setting.isThird)
        // 第三方企业用户登录
        ...
        this.isInVideoMeet = true;
        // 通知融合会商页面宽度调整
        try {
          // eslint-disable-next-line no-undef
          xyLinkVideoLogic.vuePageWidth(30)
          // eslint-disable-next-line no-undef
          xyLinkVideoLogic.vueStatusTap('meet',1)
          console.log('join ->')
        } catch (error) {
          console.log('调用 xyLinkVideoLogic.vuePageWidth() 失败：');
        }
    ...
    async handleOrientationChange(isHorizontal) {
      ...
      // const orientation = MOBILE_HORIZONTAL;
    ...
    stop() {
      try {
        // eslint-disable-next-line no-undef
        let flag = xyLinkVideoLogic.vueAndWebLimitTap()
        if(!flag){
          return
        }
      } catch (error) {
        console.log('调用 xyLinkVideoLogic.vueAndWebLimitTap() 失败：');
      }
      ...
      this.isInVideoMeet = false;
      // 通知融合会商页面宽度调整
      try {
        // eslint-disable-next-line no-undef
        xyLinkVideoLogic.vuePageWidth(0)
        // eslint-disable-next-line no-undef
        xyLinkVideoLogic.vueStatusTap('meet',0)
        console.log('stop ->')
      } catch (error) {
        console.log('调用 xyLinkVideoLogic.vuePageWidth() 失败：');
      }
    ...
    initEventListener(client) {
      ...
      client.on('disconnected', (e) => {
        ...
        try {
          ...
          console.log('调用 xyLinkVideoLogic.vueStatusTap() 失败：');
        }
      });
      ...
      client.on('layout', (e) => {
        ...
        console.log(client.getSelfRoster())
        if (this.setting.layoutMode === 'AUTO') {
          e.map(item=>{
            var rArray = []
            try {
              rArray = item.roster.extUserId.split("_");
            } catch (error) {
              console.log(error)
            }
            
            let width = parseFloat(item.rotate.width)
            let height = parseFloat(item.rotate.height)
            if(rArray.length>=2){
              if(rArray[1] == 'true' && width<height){
                item.rotate.transform = "rotateZ(90deg)"
              }
            }
          })
          this.layout = e;
          this.debounceCalcHandleUserStatus()
      ...
      client.on('call-status', (e) => {
        ...
        if (code === 'XYSDK:950518') {
          message.info(msg);
          console.log("入会成功 XYSDK:950518")
          try {
            client.setLayoutOrientation(LayoutOrientationType.MOBILE_HORIZONTAL);
            client.setLayoutOrientation(LayoutOrientationType.MOBILE_HORIZONTAL);
            client.setLayoutOrientation(LayoutOrientationType.MOBILE_HORIZONTAL);
            client.setLayoutOrientation(LayoutOrientationType.MOBILE_HORIZONTAL);
          } catch (error) {
            console.log("入会成功 :",error)
          }
      ...
      client.on('meeting-control', (e) => {
        ...
        if (!this.onhold && info) {
          ...
          try {
            // eslint-disable-next-line no-undef
            xyLinkVideoLogic.vueStatusTap('mic',this.audio !== 'unmuteAudio')
          } catch (error) {
            console.log('调用 xyLinkVideoLogic.vueStatusTap() 失败：');
          }
        }
      ...
      client.on('video-status', (e) => {
        ...
        try {
          // eslint-disable-next-line no-undef
          xyLinkVideoLogic.vueStatusTap('mic',this.video !== 'unmuteAudio')
        } catch (error) {
          console.log('调用 xyLinkVideoLogic.vueStatusTap() 失败：');
        }
      });
    ...
    // 人员视频状态
    debounceCalcHandleUserStatus: _.debounce(function(){
      this.calcHandleUserStatus()
    },500),
    calcHandleUserStatus(){
      let res = []
      let audioFlag = this.audio=='muteAudio'
      let videoFlag = this.video=='muteVideo'
      this.layout.map(item=>{
        
        res.push({
          id: item.id,
          extUserId: item.roster.extUserId,
          isLocal: item.roster.isLocal,
          displayName: item.roster.displayName,
          isVideoMute: _.has(item.roster,'isVideoMute')?item.roster.isVideoMute:videoFlag,
          isAudioMute: _.has(item.roster,'isAudioMute')?item.roster.isAudioMute:audioFlag
        })
      })
      this.xyVideoList = res
      console.log('this.xyVideoList: ',this.xyVideoList)
    },
    // 参会者信息
    handleBulkRoster(e) {
      // 参会者信息 是增量消息
    ...
    async videoOperate() {
      ...
      try {
        ...
        /* try {
          // eslint-disable-next-line no-undef
          xyLinkVideoLogic.vueStatusTap('camera',result !== 'unmuteVideo')
        } catch (error) {
          console.log('调用 xyLinkVideoLogic.vueStatusTap() 失败：');
        } */
      } catch (err) {
    ...
    async forceFullScreen(id = '') {
      ...
      console.log(id,"id")
      this.forceLayoutId = id;
      console.log(id,"id forceLayoutId",this.forceLayoutId)
    },
    ...
    switchFull(val){
      this.isFull = val
      console.log("forceLayoutId",this.forceLayoutId)
      setTimeout(()=>{
        window.dispatchEvent(new Event('resize'))
      },1000)
      try {
        // eslint-disable-next-line no-undef
        xyLinkVideoLogic.showFull(val)
      } catch (error) {
        console.log('调用 xyLinkVideoLogic.showFull() 失败：');
      }
      
      if(this.forceLayoutId && !this.isFull){
